...
$name= '';
$description = '';
$todo = base64_encode('addCategory');
if (isset($_GET['a'])) {
    $required = '';
    $id = ($_GET['a']);
    $condition = " `id` = '" . $id . "'";
    $qry = $dbComObj->viewData($conn, "category", "*", $condition);
    $num = $dbComObj->num_rows($qry);
    if ($num) {
        $todo = base64_encode('editCategory');
...
$_getC = $dbComObj->viewData($conn,"category", "*","1  order by id DESC");    
$dataAtOne = 10;
$mainPagination = "";
$data = $_REQUEST;
                            ...
                            <div class="row">
                                <div class="col-sm-4">
                                    <div class="form-group">
                                        ...
                                        <input type="text" name="cat_name" id="cat_name" class="form-control input-sm" placeholder="Enter Catgory Name" required value="<?php echo $name; ?>"/>
                                    </div>
                                </div>
                                <div class="col-sm-4">
                                    <div class="form-group">
                    ...
                    <div class="x_content">
                        ...
                        <div class="table-responsive">                            
                            <table id="countryTable" class="table table-vcenter table-condensed table-bordered">
                                ...
                                <tbody>
                                    <?php
                                    $result = $dbComObj->viewData($conn, "category", "*", "1 order by id DESC " . $mainPagination);
                                    $num = $dbComObj->num_rows($result);
                                    if ($num > 0) {
                                        $i = 0;
                                        while ($_categoryData = $dbComObj->fetch_object($result)) {
                                            $i++;
                                            if($_categoryData->status == '1')
                                            {
                                                $txtSType = '0';
                                                $txtStatus = 'Disable';
...
<a href="'.ADMIN_URL.'eMasters/manageCategory/?a='.($_categoryData->id).'#BlogData" class="btn btn-alt btn-sm btn-success" data-toggle="tooltip" title="Edit Category" data-original-title="Edit Category"><i class="fa fa-pencil mt-0"></i></a><a href="javascript:void(0)" class="btn btn-alt btn-sm btn-primary" onclick="return manageStatus('.$_categoryData->id.','.$txtSType.');" data-toggle="tooltip" data-original-title="Enable / Disable  Category"><i class="fa fa-exclamation-triangle"></i></a><a href="javascript:void(0)" class="btn btn-alt btn-sm btn-danger" onclick="return deleteCategory('.$_categoryData->id.','.$txtSType.');" data-toggle="tooltip" title="" data-original-title="Delete Category"><i class="fa fa-times"></i></a></div>';
                                            echo '<tr><td>' . $i . '</td><td>' . htmlentities($_categoryData->name) . '</td><td>' . ($_categoryData->description) . '</td><td>' . ($status) . '</td><td>' . date("M d, Y", strtotime($_categoryData->added_on)) . '</td><td>' . $_action. '</td></tr>';
                                        }
                                    }
                                    ?>
                                </tbody>
...
{
    window.location.href = '<?php echo ADMIN_URL;?>eMasters/manageCategory/?page='+page;
}
...
{
    alertBox("If you want to delete Category! All related data and Category will be removed!",'Yes, delete Category!','Category was removed from system.',e,'',btoa('deleteCategory'),'9',_urlPage);
}

function resetFilter()
{
    let _category_Id = $('#category_Id').val();
    window.location.href = '<?php echo ADMIN_URL;?>eMasters/manageCategory/';
} 

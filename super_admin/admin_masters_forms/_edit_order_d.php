...
$site_title = "Manage Edit Orders List | Salad App";
...
$todo = base64_encode('EditOrder');
//$_ord_id = isset($_REQUEST['ord_id']) ? $_REQUEST['ord_id'] : '1';
...
$_getOrderItemDetail = $dbComObj->fetch_object($dbComObj->viewData($conn, "order_item_detail", "*", "1 and order_id ='$id' order by id DESC LIMIT 0,1"));
$_getCategoryDetail = $dbComObj->fetch_object($dbComObj->viewData($conn, "category", "*", "1 and id ='".$_getOrderItemDetail->category_id."'"));
$_getProductTypeDetail = $dbComObj->fetch_object($dbComObj->viewData($conn, "Product_Type", "*", "1 and id ='".$_getOrderItemDetail->product_type_id."'"));
?>
<style>
    .container {
        top: 10%;
...
<div id="page-content">
    ...
    <div class="content-header">
        <div class="header-section">
            <h1>
                <i class="gi gi-notes_2"></i>Manage Edit Order <br><small>Manage  Edit Order!</small>
            </h1>
        </div>
                ...
                <div class="box-body">
                    <div class="x_content">
                        <form class="" id="form_employee" enctype="multipart/form-data" method="post" data-parsley-validate>
                            <div class="row">
                                <div class="col-sm-4">
                                    <p>  <strong>Order ID : </strong><?=$row['order_unique_id'];?></p> 
                                </div>
                                <div class="col-sm-4">
                                    <p>  <strong>Category Name : </strong><?=$_getCategoryDetail->name;?></p> 
                                </div>
                                <div class="col-sm-4">
                                    <p>  <strong>Order Status : </strong> <?=$_getProductTypeDetail->type;?></p> 
                                </div>
                                <div class="col-sm-4">
                                ...
                                <div class="col-sm-4">
                                    <p> <strong>Customer Email : </strong> <?=$row['customer_email'];?></p> 
                                </div>
                                <div class="col-sm-4">
                                    <p> <strong>Customer Mobile : </strong> <?=$row['customer_phone'];?></p> 
                                </div>
                                <div class="col-sm-4">
                                ...
                                <hr/>
                                <h3 style="margin: 10px 0 10px 10px;">Order Product Detail</h3>
                                <?php 
                                
                                    $_getMenuData = $dbComObj->viewData($conn, "order_item_detail", "*", "1 and order_id ='$id' order by id DESC");
                                    if ($dbComObj->num_rows($qry) > 0) {
                                        while ($_rowOrderMenuD = $dbComObj->fetch_object($_getMenuData)) {
                                            $_productData = $dbComObj->fetch_object($dbComObj->viewData($conn, "product", "*", "1 and id ='".$_rowOrderMenuD->product_id."'"));
                                            $_producId[] = $_rowOrderMenuD->product_id;
                                            echo '<div class="col-sm-12"><p> <strong> Product Name :-  </strong>'.$_productData->name.' <strong> Quantity  :- </strong> '.$_rowOrderMenuD->quantity.' <strong>Price :-</strong> '.$_rowOrderMenuD->price.'</p></div>';
                                        }
                                    }
                                    $allPrdId = implode(",",$_producId);
                                    //print_r($allPrdId);
                                    ?>
                            ...
                            <div class="row">
                                <div class="col-sm-4 col-sm-offset-4">
                                    ...
                                    <input type="hidden" name="product_type_id" id="product_type_id" value="<?=$_getOrderItemDetail->product_type_id;?>" />
                                    <input type="hidden" name="category_id" id="category_id" value="<?=$_getOrderItemDetail->category_id;?>" />
...
$("#search_product").autocomplete({
    source: function (request, response) {
        $.getJSON("<?php echo ADMIN_URL; ?>_controller/_ss_subscription_order.php?todo=<?php echo base64_encode("searchMenuItemEdit"); ?>&cat=<?=$_getOrderItemDetail->category_id;?>&ptype=<?=$_getOrderItemDetail->product_type_id;?>&addedPrd=<?=$allPrdId;?>", {request: request},
        response);
    },
    open: function () {
